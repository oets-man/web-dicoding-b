...
function renderBooks(books) {
  ...
  read.innerHTML = '';
  read.parentElement.parentElement.style.overflowX = 'unset';

  const unread = document.getElementById('table-unread');
  unread.innerHTML = '';
  unread.parentElement.parentElement.style.overflowX = 'unset';

  for (const book of books) {
    const bookElement = makeBook(book);
    if (!book.isRead) {
      unread.append(bookElement);
      unread.parentElement.parentElement.style.overflowX = 'scroll';
    } else {
      read.append(bookElement);
      read.parentElement.parentElement.style.overflowX = 'scroll';
    }
  }

  // if no data
  const noData = `<tr><td colspan=5 style="text-align:center; color: red; font-style:italic">Tidak data untuk ditampilkan</td></tr>`;
  if (read.innerHTML == '') {
    read.innerHTML = noData;
  }
  if (unread.innerHTML == '') {
    unread.innerHTML = noData;
  }
}
